 ...
 * @property {DieuKienChay} khi dấu hiệu nhận biết cần phục hồi, ví dụ ô đăng nhập hiện lại
 * @property {Buoc[]} buoc
...
export function kiemTraKichBan(kb) {
  ...
  ;(kb.phucHoi ?? []).forEach((p, i) => {
    ...
    if (!p.khi.selector.trim()) loi.push(`${nhan} ("${p.ten}"): thiếu selector dấu hiệu`)
